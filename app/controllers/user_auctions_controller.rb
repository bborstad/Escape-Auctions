class UserAuctionsController < ApplicationController
  ...
  def index
    ...
    def new
      ...
      def create
        auction = current_user.auctions.build(params.require(:auction).permit(:title, :description, :starting_bid, :buy_now_price, :expire_date, :image, :status))
        respond_to do |format|
          format.html do
            if auction.save
    ...
    def update
        auction = Auction.find(params[:id])
            respond_to do |format|
                format.html do
                if auction.update(params.require(:auction).permit(:title, :description, :starting_bid, :buy_now_price, :expire_date, :image, :status))
